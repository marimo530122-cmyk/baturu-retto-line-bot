...
### AIが詐欺グループの口座まで聞き出す(`lib/elicit.js`)

「AIに代わってもらう」を押したあと、AIは時間を稼ぐだけでなく、**警察に渡す手がかりを自分から聞き出しにいく**。
海外の詐欺対策ハニーポットOSS(ScamIntelli、Agentic-Honeypot など)で使われている考え方を取り入れた(コードは取り込まず、設計だけ)。

- **足りない手がかりを狙って聞く**: 銀行名と支店 → 口座番号 → 相手の名前 → 金額 → 受け渡しの日時と場所 → 折り返しの電話番号 の順に、
  まだ聞けていないものを1つずつ、メモを取るふりで聞き返す(「ごめんなさい、メモしますから、どこの銀行の、何支店でしたっけ」)。
  振込の話が出ていないのに口座を聞く、のような不自然な質問はしない。聞き出しと時間稼ぎは交互にして、しつこくならないようにする。
- **相手の様子を見る(Jev)**: 相手が「協力的 / 怪しみ始めた・いら立っている / 切ろうとしている」をJevに聞き、
  怪しまれたら聞くのをやめて「ちゃんと聞いてますよ。年を取ると物覚えが悪くて」となだめ、つなぎ止める。
  Jevが無い・間に合わないとき(既定1.5秒)は、「さっきから本当に聞いてるの」のような言葉から判断する。
- **同じ返事を繰り返さない**: 直前と同じ返事は機械だとばれるので、別の言い方に替える。
- **約束はしない**: 聞き出しても「振り込みます」「渡します」「行きます」とは言わない(言いそうになったら読み上げずに差し替える)。
...
## 固定電話で使う(`docs/landline.html`)

表示画面のない家の固定電話は、NTTの「ボイスワープ」(着信転送)で戦国シールドのTwilio番号に回し、AIが先に電話に出る形にする。
本人は画面もボタンも使わず、相手が口座や会う日時・場所を言ったら、家族にLINEと自動電話で知らせる。
「すべての電話にAIが先に出る(無条件転送)」と「出られないときだけAIが出る(無応答時転送)」の2通りの設定、
...
110番の台本で警察が動けるか、口座・電話番号の届け先、お年寄りと家族への伝え方などを、チェックを付けながら聞ける。
聞いた内容は質問番号を付けてClaudeに送り、例文(まず holdout)と検知ルールの改良に使う。
...
| `JEV_ENGAGEMENT_TIMEOUT_MS` | | AIが代わりに話している間、相手の様子をJevに聞く待ち時間(既定 1500ms) |
